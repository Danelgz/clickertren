...
// ================================
// 40 MEJORAS PERSONALIZADAS ÚNICAS
// ================================
const customNames = [
...
// ================================
// TREN CLICK CON PUNTOS FLOTANTES (CLICK NORMAL Y MANTENIDO)
// ================================
...
train.onmousedown = (e)=>{
    const addPoints = (event)=>{
        ...
        floatSpan.style.left = (event.clientX + 5) + 'px';
        floatSpan.style.top = (event.clientY - 20) + 'px';
        floatSpan.style.color = '#FFD93D';
        floatSpan.style.fontWeight = 'bold';
        floatSpan.style.fontSize = '1.2rem';
        floatSpan.style.pointerEvents = 'none';
        floatSpan.style.transition = 'all 0.8s ease-out';
        document.body.appendChild(floatSpan);

        setTimeout(()=>{
            floatSpan.style.top = (event.clientY - 50) + 'px';
    ...
    addPoints(e); // click inicial
    clickInterval = setInterval(()=>addPoints(e), 200); // velocidad moderada
};

train.onmouseup = train.onmouseleave = ()=>{
    ...
    upgrades.forEach((up)=>{
        ...
        div.innerHTML=`
            <div>
                ...
                ${lockedByRank?`<div>🔒 Requiere rango: ${ranks[up.requiredRank].name}</div>`:''}
            </div>
...
function renderRanks(){
    ...
    ranks.forEach((r,i)=>{
        ...
        if(i < playerRankIndex){
            ...
            const previousRankOwned = (i === 0) || (playerRankIndex === i - 1);
...
function renderBoosters(){
    ...
    boosters.forEach((b)=>{
        ...
        div.className='upgrade';
        div.innerHTML=`
            <div style="line-height:1.4; margin:2px 0;"><strong>${b.name}</strong></div>
            <div style="line-height:1.4; margin:2px 0;">x${b.multiplier} durante ${b.duration}s</div>
            ${remainingTime>0?`<div style="line-height:1.4; margin:2px 0;">⏱ ${remainingTime}s restantes</div>`:''}
            <div class="${canBuy?'price-can-buy':'price-cant-buy'}" style="line-height:1.4; margin:2px 0;">Precio: ${formatNumber(b.price)}</div>
            ${lockedByRank?`<div style="line-height:1.4; margin:2px 0;">🔒 Requiere rango: ${ranks[b.requiredRank].name}</div>`:''}
            <button ${canBuy?'':'disabled'}>Comprar</button>
        `;
